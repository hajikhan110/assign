...
//POOR APROACH
var c_username = ['KAREEM', 'raheem', 'saleem', 'muheemun', 'jabar'];
var new_users = ['admin', 'raheem', 'kareem'];
//const username : string[] =['kareem','raheem','saleem','salam','muheemun','admin'];
for (var i1 = 0; i1 < new_users.length; i1++) {
    for (var i2 = 0; i2 < c_username.length; i2++) {
        // if(c_username[i1]=== new_users[i2]){
        if (c_username[i1] === new_users[i2] || c_username[i1].toUpperCase() === new_users[i2]) {
            console.log("need to enter a new username ".concat(c_username[i1]));
        }
        else {
            console.log("username ".concat(c_username[i1], " is available."));
